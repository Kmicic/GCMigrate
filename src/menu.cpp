...
#if (DEBUG_ENABLE==1)
      Serial.println("menu.ino -> void serviceSett(118)");
      Serial.print("arrowPos ="); Serial.println(arrowPos);
    ...
    case 3:
      switch (thisMode) {
        ...
        case 2:  // неделька
          ...
          break;
        case 3:
          incr(&setChannel.sensor, dir, 6); //dodatkowy sensor numer 6 Pressure
          if (setChannel.sensor==6) {
          setChannel.threshold = round(sensorVals[setChannel.sensor]/100);
          setChannel.thresholdMax = round(sensorVals[setChannel.sensor]/100);
          } 
          else
          {
